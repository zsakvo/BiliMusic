...
class StringFormatUtils {
  ...
  // byte数转文件大小
  static byteToSizeString(int size) {
    if (size < 1024) {
      return "$size B";
  ...
  static List<Map<String, dynamic>> splitStringWithEmTags(String str) {
    final inputString = replaceAllHtmlEntitiesToCharacter(str);
    List<Map<String, dynamic>> result = [];

    RegExp regex = RegExp(r'<em.*?>(.*?)<\/em>', multiLine: true, dotAll: true);
    int lastIndex = 0;

    for (RegExpMatch? match in regex.allMatches(inputString)) {
      if (match != null) {
        String beforeText = inputString.substring(lastIndex, match.start);
        String emText = match.group(1)!;

        result.add({"content": beforeText.trim(), "isKey": false});
        result.add({"content": emText.trim(), "isKey": true});

        lastIndex = match.end;
      }
    }

    String afterText = inputString.substring(lastIndex);
    result.add({"content": afterText.trim(), "isKey": false});

    return result;
  }

  static String formatNumberWithUnit(int number) {
    if (number < 1000) {
